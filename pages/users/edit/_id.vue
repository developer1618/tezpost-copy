          ...
          <form class="space-y-6" @submit.prevent="save">
            <div class="flex flex-wrap w-full">
              ...
              <div class="w-full lg:w-1/3 md:w-1/2 md:px-3 py-2">
                ...
                <div class="mt-1 flex">
                  <span class="w-20 pt-2 pl-2 bg-gray-300 rounded-tl rounded-bl"
                    >+992</span
                  >
                  <input
                    v-model="user.phone"
                  ...
                  <div class="row-span-2 col-span-2 relative group">
                    <label for="image" class="cursor-pointer"
                      ><img
                        :src="
                          user.avatar
                            ? 'http://test-api.tezpost.ru' + user.avatar
                            : '/img/upload_plus.svg'
                        "
                        class="w-full h-full"
                    /></label>
...
export default {
  ...
  methods: {
    init() {
      this.$axios
        .get(`/dashboard/clients/${this.$route.params.id}`)
        .then((res) => {
    ...
    save() {
      this.$axios
        .put(`/dashboard/clients/${this.user.id}`, this.user)
        .then((res) => {
          this.$swal({
            icon: "success",
            title: "Успешно",
